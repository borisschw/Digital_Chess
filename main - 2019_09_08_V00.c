...
//        for (int i = 3; i < recieved_data.length; i++ )
//        {
//           calc_checksum = calc_checksum + temp_data[i];
//        }

//        if (recieved_data.checksum != calc_checksum)
//        {
//          NRF_LOG_ERROR("ERROR in Checksum");
//        }
//        else
//        {
//          NRF_LOG_ERROR("Checksum OK");
//        }
...
//uint16_t length = sizeof(init_board_state);
//pboard_state = (uint8_t*)&init_board_state;
//ret = ble_nus_data_send(&m_nus, pboard_state, &length, m_conn_handle); 
...
//            for (int i = 3; i < recieved_data.length; i++ )
//            {          
//              recieved_data.data[i] = temp_data[i];
////              current_state_board[i] = temp_data[i];
//            }
//        break;
//        case 0x81:  //start of game
//          start_of_game_flag = 1;
//          white_turn = true;
//
//        break;
//        case 0x82:  //end of game
//
//        break;     
//        }
//
//        recieved_data.checksum = temp_data [i + 1];                
//        calc_checksum = recieved_data.header + recieved_data.command + recieved_data.length;

//        nrf_delay_ms(10)true
...
int read_adc(_Bool *val)
{
    ...
    if (m_buffer_pool[0][1] < THRESHOLD) // AN0
        val[0] = true;
    else 
        val[0] = false;
      

    if (m_buffer_pool[0][0] < THRESHOLD)//AN1
        val[1] = true;
    else 
        val[1] = false;  
