...
#define THRESHOLD 15
#define EOM_THRESHOLD  15 // 50*10mSec =500msec
...
int read_board_position(void)
{
  ...
  */
  ...
  { 
    ...
    nrf_delay_us(30);
    read_adc(ADC_Val);    
    position_test[index_B * 2] = ADC_Val[0];
    position_test[(index_B * 2) + 1] = ADC_Val[1];
    if (ADC_Val[0] == true)
...
{
  int i,j;
  volatile static uint8_t line;
  volatile static uint8_t column;
  volatile static uint8_t current_bit;
  volatile static uint8_t new_bit;
    ...
    case castling_phase_2:
      if ((change_val > 0x7F)&&(piece_type==0)) 
      {
        if (white_turn) 
        {
          ...
          if (((new_state[0] & 0x60) == 0x60) || ((new_state[0] & 0x0C) == 0x0C))
          {
            move_case = start_move;
            global_index =move_case;
            flag_castling = false;
        ...
        else 
        {
          ...
          if (((new_state[7] & 0x60) == 0x60) || ((new_state[7] & 0x0C) == 0x0C)) 
          {
            move_case = start_move;
            global_index =move_case; 
            flag_castling = false;
