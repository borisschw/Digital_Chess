...
// 2019_09_08_V03  fix algo and add End of move                                                   //
...
#define THRESHOLD 20
#define EOM_THRESHOLD  50 // 50*10mSec =500msec
...
/*-- Global Variables--*/
volatile uint8_t change_array[8][2];
...
int read_board_position(void)
{
  ...
  */
  ...
  { 
    ...
    if(((index+1) % BITS_IN_BYTE) == 0) // if scanned a line - update the array
    {
      new_state[((2 * index) / BITS_IN_BYTE)-1] = line_val0;
      new_state[((2 * index) / BITS_IN_BYTE)] = line_val1;
      line_val0 =0;
      line_val1 =0;
    } 
...
int compare_block(void)//(uint8_t *current_state, uint8_t *new_state)
{
  ...
  volatile uint8_t change_location;
  volatile uint8_t change;
  volatile _Bool flag_new_change;
  ...
  */
  ...
  for(change = 0; change < MAX_NUMBER_OF_CHANGES; change++) 
  {
    if (change_array[change][0]!=INIT_CHANGE_VALUE)
    {
      ...
      {
        move_algo(change_array[change][0] ,false);
...
int move_algo(uint8_t change_val, _Bool reset) 
{
  ...
  volatile uint8_t place;
  volatile uint8_t piece_type;
  ...
  place = ((change_val & 0x70) >> 1) + (change_val & 0x07); //0-64 posision and type
    ...
    case capture_phase:
        if ((change_val > 0x79)
          &&(place == type_and_place[1][0])) 
          {
            ...
            move_case = start_move;
            global_index =move_case;
            for (uint8_t i=0; i<3; i++)    // clear type_and_place array 
