...
#define THRESHOLD 7
#define EOM_THRESHOLD  20 // 50*10mSec =500msec
...
void send_err(uint8_t err_type );
...
int move_algo(uint8_t change_val, _Bool reset) 
{
  ...
  switch (move_case)
  {
    ...
    case castling_phase_2:
      ...
      else if ((change_val < 0x7F)&&((piece_type==ROOK_B)||(piece_type==ROOK_W)))
      {
         current_state_board[place] =0;
      }
      else 
